...
// TestIsOvertimeSubmitted ...
func TestIsOvertimeSubmitted(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFunc: func(e uuid.UUID, p time.Time) {
				...
				mocks.ExpectQuery(`SELECT 1 FROM overtimes WHERE date = $1 AND employee_id = $2 AND is_deleted = $3 LIMIT 1`).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := r.IsOvertimeSubmitted(context.Background(), tt.eid, tt.d)
			if tt.wantErr {
